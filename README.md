...
> Status: `v0.1.0` publicado. A versão atual é forte em leitura e diagnóstico. Escrita existe em modo experimental e ainda será simplificada em uma próxima versão.
...
- Escrita experimental: `leads2b_update_customer`.

Veja a lista completa em [docs/MCP-TOOLS.md](docs/MCP-TOOLS.md).

## Escrita

Na `v0.1.0`, ferramentas de escrita ficam desabilitadas por padrão:

```txt
LEADS2B_ENABLE_WRITE_TOOLS=true
```

A única ferramenta disponível é `leads2b_update_customer`, ainda experimental. A próxima versão deve trocar esse modelo por um modo operacional mais direto para CRUD real.
